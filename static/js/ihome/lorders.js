...
$(document).ready(function(){
    ...
    $.get('/home/myorders/',function(data){
        if(data.code == '200'){
            for(var i=0; i<data.orders_list.length; i++){

                console.log(data.orders_list[i])
                var orders_li = ''
                orders_li += '<li order-id='+ data.orders_list[i].order_id +'><div class="order-title"><h3>订单编号：'+ data.orders_list[i].order_id +'</h3>'
                orders_li += '<div class="fr order-operate"><button type="button" class="btn btn-success order-accept" data-toggle="modal" data-target="#accept-modal">接单</button>'
                orders_li += '<button type="button" class="btn btn-danger order-reject" data-toggle="modal" data-target="#reject-modal">拒单</button></div></div>'
                ...
                orders_li += '<li>客户评价：'+ data.orders_list[i].comment +'</li>'
                orders_li += '</ul>'
                orders_li += '</ul></div></div></li>'

                $('.orders-list').append(orders_li)
                if(data.orders_list[i].status != 'WAIT_ACCEPT'){
                    $('li[order-id='+ data.orders_list[i].order_id +'] button').hide()
//                    $('.order-reject').hide()
                }
            ...
            $(".order-reject").on("click", function(){
                var orderId = $(this).parents("li").attr("order-id");
                $(".order-reject").attr("order-id", orderId);
            });
        }
    });
